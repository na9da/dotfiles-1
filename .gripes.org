...
** Chrome OS
   - can't disable internal keyboard inside chromeos, just in crouton
   - ssh client doesn't support alt-v nor fullscreening properly
     - mosh client works much better
   - dev mode defaults to garbage security
     - easy to fix, but holy smokes what a bad policy
   - chrome key bindings are cringeworthy
     - cannot override ctrl-n
     - ctrl-shift-tab is objectively awful
     - just use firefox inside crouton
** Emacs
*** elisp
   - lisp-2 is awful
   - no FFI
...
* Hardware
** Thinkpad X301
   - multiple batteries: system shuts down when one is full and one is empty
   - screen is very dim
   - screen is low-res
